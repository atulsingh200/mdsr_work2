...
from followup_data.negatives import with_hard_negatives, with_random_negatives
...
def main() -> None:
    ...
    p.add_argument(
        "--hard-negatives",
        action="store_true",
        help="BM25-mine hard negatives instead of sampling randomly. "
        "Requires `pip install '.[hard-negatives]'`.",
    )
    args = p.parse_args()
    ...
    if args.hard_negatives:
        triples = list(with_hard_negatives(pairs, k=args.k))[: args.n]
    else:
