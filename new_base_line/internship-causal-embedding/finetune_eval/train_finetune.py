...
from finetune_eval.losses import HardNegativeInfoNCELoss, HardOnlyInfoNCELoss  # noqa: E402
...
def main() -> None:
    ...
    ap.add_argument("--no-inbatch", action="store_true",
                    help="Use HardOnlyInfoNCELoss: K hard negatives only, no in-batch term. "
                         "Matches the CrossEncoder's training signal for a fair ablation.")
    ap.add_argument("--results-suffix", default="",
                    help="Append this suffix to the results subdir name, e.g. '_no_inbatch'.")
    args = ap.parse_args()
    ...
    device = auto_device()
    # Checkpoints go into a separate subdir when --results-suffix is set so the
    # ablation run never overwrites the original baseline checkpoints.
    ckpt_dir = Path(args.results_dir) / (args.dataset + args.results_suffix)
    # Mining artifacts (train_pairs.jsonl, hard_negatives.npy) always come from
    # the base results dir — they are shared across all runs.
    mining_dir = Path(args.results_dir) / args.dataset
    ...
    train_pairs_path = mining_dir / "train_pairs.jsonl"
    hard_neg_path = mining_dir / "hard_negatives.npy"
    ...
    # ---- Loss + optimizer ----
    LossCls = HardOnlyInfoNCELoss if args.no_inbatch else HardNegativeInfoNCELoss
    print(f"[loss] {LossCls.__name__}  ({'no in-batch' if args.no_inbatch else 'in-batch + hard negs'})")
    loss_fn = LossCls(
    ...
    cfg = {
        ...
        "device":               str(device),
        "no_inbatch":           args.no_inbatch,
        "loss":                 LossCls.__name__,
    }
