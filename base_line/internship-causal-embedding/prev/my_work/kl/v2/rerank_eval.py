...
def ranks_from_scores(score_mat: np.ndarray) -> np.ndarray:
    """Rank of the diagonal (true positive) in each row. Higher score = better."""
    N = score_mat.shape[0]
    diag = np.diagonal(score_mat)[:, None]
    return (score_mat > diag).sum(axis=1).astype(np.float64) + 1.0
...
def main() -> None:
    ...
    if cpool < N:
        ...
        # For each query i: sample (cpool-1) distractors + always include true positive i
        cand_idx = np.zeros((N, cpool), dtype=np.int32)
        for i in range(N):
            distractors = rng_cpool.choice(N - 1, size=cpool - 1, replace=False)
            distractors[distractors >= i] += 1          # exclude self, shift to avoid gap
            cand_idx[i, 0] = i                          # gold always at position 0
            cand_idx[i, 1:] = distractors
    ...
    for pool in args.pool:
        ...
        for blend in args.blend:
            ...
            for i in range(N):
                ...
                if cand_idx is not None:
                    pos_in_pool = np.where(topk[i] == 0)[0]   # col 0 = gold
                else:
