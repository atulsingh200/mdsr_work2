...
Identical training setup to the 1x cross-encoder (cross_encoder/train_ce.py)
so results are directly comparable:
  ...
  * Early stopping: stop if val AUC fails to improve by threshold for patience epochs
...
from dataset_ce import PairLabelDataset, load_pairs, make_collate_fn  # reuse 1x dataset
...
def main() -> None:
    ...
    ap.add_argument("--batch-size", type=int, default=64,
                    help="Smaller default than 1x CE to fit the 2x model on GPU.")
    ...
    # num_workers>0 forks the full dataset into each worker — on large datasets
    # (workflow 200k pairs) that multiplies CPU RAM by num_workers+1 and risks OOM.
    # Cap at 2 workers; the tokenizer/collate is the bottleneck anyway.
    safe_workers = min(args.num_workers, 2)
    pin = torch.cuda.is_available()
    train_loader = DataLoader(
        train_ds, batch_size=args.batch_size, shuffle=True,
        num_workers=safe_workers, pin_memory=pin, collate_fn=collate, drop_last=False,
    )
    ...
    log(f"[train] dataset={args.dataset} total_steps={total_steps} "
        f"warmup={warmup_steps} batch_size={args.batch_size} grad_accum={args.grad_accum} "
        f"effective_batch={effective_batch} epochs={args.epochs} max_length={args.max_length} "
        f"params={n_params:,}  early_stop=(thresh {args.early_stop_threshold}, "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for micro_step, batch in enumerate(train_loader):
            ...
            epoch_loss_sum += float(loss.item()) * args.grad_accum
            epoch_steps += 1
