...
# run_ce_baselines.sh
#
# CROSS-ENCODER baseline sweep: 7 datasets × 6 models = 42 training runs,
# distributed across 4 A100 GPUs (1 job per GPU at a time). After all training
# completes, OOD eval runs bounded to 4 concurrent (1/GPU), then results are
# aggregated into final_results.json.
#
# Mirror of run_baselines.sh (the bi-encoder sweep) — same orchestration, same
# fixes (cuda:0 device, DeBERTa --no-amp, serial pre-download, bounded eval),
# but trains a single-encoder cross-encoder and writes to a separate folder.
#
# Usage (launch EXACTLY ONCE — it is idempotent on re-run):
...
TRAIN_SCRIPT=$REPO/src/classifier/crossencoder/train_ce_baselines.py
EVAL_SCRIPT=$REPO/src/classifier/crossencoder/eval_ood_ce.py
...
# Dedicated sweep home (separate from the bi-encoder baseline_sweep).
SWEEP_HOME=/mnt/localssd/crossencoder_sweep
