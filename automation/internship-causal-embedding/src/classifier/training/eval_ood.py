#!/usr/bin/env python3
"""OOD evaluation for baseline sweep checkpoints.

Loads a trained DirectionalClassifier from a run directory and scores
two held-out test sets:

  1. test_samples_milan.json
       Flat list of {label1, label2, text1, text2} pairs drawn from a
       multi-step workflow.  All pair directions are scored; the tournament
       + net-precedence algorithm (order_events.py) recovers the full event
       ordering.  Ground truth = ascending label order (t1 < t2 < ... by
       numeric suffix).
...
Usage:
  python eval_ood.py --run-dir /mnt/localssd/baselines_output/aep_dataset/bge-large-en-v1.5
"""
...
def score_pairs(
    ...
    """Return P(text_a precedes text_b) for each pair."""
    ...
    for s in range(0, len(texts_a), BATCH_SIZE):
        ...
        enc1 = tokenizer(ba, padding=True, truncation=True,
                         max_length=max_len, return_tensors="pt")
        enc2 = tokenizer(bb, padding=True, truncation=True,
                         max_length=max_len, return_tensors="pt")
        enc1 = {k: v.to(device) for k, v in enc1.items() if k in _SAFE_ENC_KEYS}
        enc2 = {k: v.to(device) for k, v in enc2.items() if k in _SAFE_ENC_KEYS}
        with ctx():
...
# ---------------------------------------------------------------------------
# Milan multi-step ordering (order_events.py algorithm)
# ---------------------------------------------------------------------------
...
def eval_milan(
    ...
    # Pairwise accuracy: for each directed pair (label_i -> label_j) where
    # ground truth says i comes before j, count how many the model also predicts
    # i -> j (score > 0.5).
    n_pairs_correct = 0
    n_pairs_total = 0
    for r in scored_pairs:
        ...
        # Only count pairs where labels differ in ground-truth rank
        if i_rank == j_rank:
            continue
        n_pairs_total += 1
        gt_positive = i_rank < j_rank  # i precedes j in ground truth
...
def eval_ajo(
    ...
    # Flatten all 6 directed pairs across all samples for a single batched call
    all_a: list[str] = []
    all_b: list[str] = []
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="OOD evaluation for a baseline sweep checkpoint.",
    ...
    ap.add_argument(
        "--milan-json",
        default="/mnt/localssd/test_samples_milan.json",
        help="Flat Milan pairs JSON.",
    )
    ap.add_argument(
        "--ajo-json",
        default="/mnt/localssd/ajo_orchestrated_workflows_flat.json",
        help="AJO 30-sample 3-step ordering JSON.",
    )
    ap.add_argument("--batch-size", type=int, default=32)
    ap.add_argument("--device", default=None,
                    help="torch device string; auto-detected if omitted.")
    args = ap.parse_args()
    ...
    cfg = ckpt["cfg"]
    # Match the precision used at train time (fp32 for DeBERTa-v3) so inference
    # is numerically consistent with training.
    ...
    print(
        f"base_model={base_model}  head={cfg.get('head')}  "
    ...
    # Force fp32 master weights before loading state_dict — some backbones load
    # in fp16 (e.g. DeBERTa-v3), which would mismatch the fp32 head.
    model = model.float()
    ...
    print("  per_event:", flush=True)
    for lbl, ev in milan_results["per_event"].items():
        print(
            f"    {lbl}: wins={ev['tournament_wins']} net={ev['net_precedence']:.3f} "
            f"pred_rank={ev['predicted_rank']} true_rank={ev['true_rank']}",
            flush=True,
        )
    ...
    for s in ajo_results["per_sample"]:
        status = "OK" if s["correct"] else "X"
        print(f"  [{status}] id={s['id']}  predicted={s['predicted_order']}", flush=True)

    # -- write results --
    # Remove verbose scored_pairs from ood_results to keep file compact; keep per_event
