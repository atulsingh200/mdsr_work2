#!/usr/bin/env bash
# End-to-end ensemble cross-encoder pipeline for the `aep_dataset` corpus.
# Mirrors run_all.sh but with aep_dataset-specific paths and bi-encoder ckpt.
#   1. mine semantic hard negs (train+val)        -> data/aep_dataset_hard_neg_semantic
#   2. mine in-domain hard negs (train+val)        -> data/aep_dataset_hard_neg_indomain
#      (uses bi-encoder runs/classifier/best_mlp_bge_small_aep/best.pt)
...
#
# Usage:
#   bash src/classifier/crossencoder/run_all_aep_dataset.sh
#   GPU=1 bash src/classifier/crossencoder/run_all_aep_dataset.sh          # pick GPU
#   SKIP_MINING=1 bash src/classifier/crossencoder/run_all_aep_dataset.sh  # reuse mined data
set -euo pipefail
...
SRC=data/aep_dataset
SEM=data/aep_dataset_hard_neg_semantic
IND=data/aep_dataset_hard_neg_indomain
RUNS=runs/crossencoder_aep_dataset
# Bi-encoder checkpoint trained on aep_dataset (used for in-domain mining).
BIENCODER_CKPT=runs/classifier/best_mlp_bge_small_aep/best.pt
...
python3 -m src.classifier.crossencoder.ensemble_eval \
  ...
  --baseline-metrics runs/classifier/best_mlp_bge_small_aep/test_metrics.json \
