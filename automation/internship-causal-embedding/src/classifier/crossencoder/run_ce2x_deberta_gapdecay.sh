...
DATA_DIR="$REPO_ROOT/data/merged_gapdecay_procedural"
...
echo "===== training CE2x-deberta-v3-large on merged_gapdecay_procedural ====="
python3 -m src.classifier.crossencoder.train_ce2x \
    --data-dir "$DATA_DIR" \
    --split-prefix "" \
    --out-dir "$RUNS" --run-name merged_gapdecay_procedural \
    $CE2X_ARGS \
    2>&1 | tee "$RUNS/ce2x_deberta_merged_gapdecay_procedural_stdout.log"
