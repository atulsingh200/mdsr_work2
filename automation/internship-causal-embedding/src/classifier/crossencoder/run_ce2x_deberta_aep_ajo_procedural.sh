...
DATA_DIR="$REPO_ROOT/data/aep_ajo_procedural_workflow"
...
echo "===== training CE2x-deberta-v3-large on aep_ajo_procedural_workflow ====="
python3 -m src.classifier.crossencoder.train_ce2x \
    --data-dir "$DATA_DIR" \
    --split-prefix "" \
    --out-dir "$RUNS" --run-name aep_ajo_procedural_workflow \
    $CE2X_ARGS \
    2>&1 | tee "$RUNS/ce2x_deberta_aep_ajo_procedural_workflow_stdout.log"
