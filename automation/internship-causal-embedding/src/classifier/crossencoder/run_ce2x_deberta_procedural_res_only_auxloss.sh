#!/usr/bin/env bash
# DeBERTa-v3-large cross-encoder trained on procedural-only data (no AEP/AJO)
...
DATA_DIR="$REPO_ROOT/data/procedural_res_only"
...
echo "===== training CE2x-deberta-v3-large + reasoning aux-loss on procedural_res_only ====="
python3 -m src.classifier.crossencoder.train_ce2x_reasoning \
    --data-dir "$DATA_DIR" \
    --split-prefix "" \
    --out-dir "$RUNS" --run-name procedural_res_only_auxloss070 \
    $CE2X_ARGS \
    2>&1 | tee "$RUNS/ce2x_deberta_procedural_res_only_auxloss_stdout.log"
