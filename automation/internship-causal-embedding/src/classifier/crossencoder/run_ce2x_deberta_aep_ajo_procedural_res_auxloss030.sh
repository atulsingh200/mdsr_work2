...
echo "===== training CE2x-deberta-v3-large + reasoning aux-loss (cls=0.7, dec=0.3) on aep_ajo_procedural_res ====="
python3 -m src.classifier.crossencoder.train_ce2x_reasoning \
    --data-dir "$DATA_DIR" \
    --split-prefix "" \
    --out-dir "$RUNS" --run-name aep_ajo_procedural_res_auxloss030 \
    $CE2X_ARGS \
    2>&1 | tee "$RUNS/ce2x_deberta_aep_ajo_procedural_res_auxloss030_stdout.log"
