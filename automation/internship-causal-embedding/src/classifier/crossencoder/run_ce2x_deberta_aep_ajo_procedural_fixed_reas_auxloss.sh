...
echo "===== training CE2x-deberta-v3-large + reasoning aux-loss on aep_ajo_procedural_fixed_reas ====="
python3 -m src.classifier.crossencoder.train_ce2x_reasoning \
    --data-dir "$DATA_DIR" \
    --split-prefix "" \
    --out-dir "$RUNS" --run-name aep_ajo_procedural_fixed_reas_auxloss070 \
    $CE2X_ARGS \
    2>&1 | tee "$RUNS/ce2x_deberta_aep_ajo_procedural_fixed_reas_auxloss_stdout.log"
