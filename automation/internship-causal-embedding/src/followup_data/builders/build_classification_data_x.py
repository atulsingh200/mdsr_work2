...
Pipeline:
  ...
  6. Re-chunk each doc with a sliding window (window=256, stride=128) using the
     encoder's HuggingFace tokenizer.
  7. Doc-level train/val/test split within each sub-chapter (80/10/10, seed=42).
  8. Emit adjacent-tier pair samples both directions; pair goes to train only if
     both docs are in train (no cross-split leakage). Same-tier pairs dropped.
  9. Write JSONL: one file per split in OUT_DIR.

Invoke:
  python3 classification/build_directional_dataset.py [--window 256] [--stride 128]
          [--tokenizer sentence-transformers/all-mpnet-base-v2]
          [--corpus aep_docs_collection_v_24-03-2026.json]
          [--out_dir classification/data]
...
def main() -> None:
    ...
    # -- map leaves to (url, subchapter, tier) --
    resolved: list[dict] = []
    ...
    for leaf in leaves:
        ...
        # find URL in corpus by trying candidates
        url_found = None
    ...
    print(f"[resolve] {len(resolved)} docs resolved, {len(url_misses)} URL misses, {len(subchapter_misses)} subchapter misses", file=sys.stderr)
    ...
    try:
        if args.split_level in ("sub_chapter", "tier"):
            ...
            if cap is not None:
                # cap is on SAMPLES; convert to max tuples
