...
src/followup_data/
  ...
  loaders/
    ...
    aep_causal_classification.py  📝 classification pairs (data not shipped, see below)
    _manual.py         📝 stubs for datasets needing manual setup

src/followup_data/
  classification_base.py       ClassificationExample, BaseClassificationDataset
  classification_registry.py   @register_classification, load_classification()
  builders/
    build_classification_data.py   `build-classification-data` CLI (corpus → JSONL)
...
src/classifier/       # directional classification pipeline
  model.py            DirectionalClassifier (dual encoder + [A;B;A-B;A*B])
  heads.py            LinearHead, MLPHead, build_head (pluggable heads)
  training/
    data.py           PairDataset + Collate (reads classification JSONL)
    train.py          Training loop + `train-classifier` CLI entry
...
Classification datasets use a separate abstraction (`ClassificationExample`,
`BaseClassificationDataset`) because each row carries a binary label — unlike
retrieval pairs where every row is implicitly positive.

### AEP Causal Classification

Tier-based directional classification on AJO tutorial segments. Each row is a
`(text_1, text_2, label)` triple: label=1 means text_1's tier causally precedes
text_2's tier in a 15-tier dependency hierarchy. Segments are sentence-based
(2–5 sentences). All C(15,2)=105 tier-pair combinations, single-direction,
capped at 1,500 per tier-pair.
...
**Data generation.** The JSONL files are not shipped with the repo due to size
(~180 MB). Generate them from the AEP docs corpus using the included builder:

```bash
uv pip install '.[training]'   # needs transformers for tokenizer

# v6 configuration: sentence-based, all tier-pairs, single-direction, capped
uv run build-classification-data \
    --corpus path/to/aep_docs_collection.json \
    --out-dir data/aep_causal_classification \
...
This produces:
```
data/aep_causal_classification/
  directional_train.jsonl     (110,775 rows)
  directional_val.jsonl       (6,004 rows)
  directional_test.jsonl      (6,004 rows)
  directional_manifest.json   (tier table + generation config)
```

The builder requires the AEP docs corpus JSON (internal) and fetches the
AJO tutorial TOC from Adobe's GitHub. Full flag reference:
`uv run build-classification-data --help`.

Each JSONL row has the schema:
```json
{
  "text_1": "...", "text_2": "...", "label": 1,
  "tier_1": 1, "tier_2": 3,
  "sub_1": "2a", "sub_2": "14b",
  "url_1": "https://...", "url_2": "https://..."
}
```
...
```python
from followup_data import load_classification

ds = load_classification("aep_causal_classification", root="./data", split="train")
for ex in ds.head(3):
    print(ex.text_1[:80], "->", ex.label)
    print(ex.metadata)  # tier_1, tier_2, sub_1, sub_2, url_1, url_2
```
...
1. Create `src/followup_data/loaders/your_dataset.py`.
2. Subclass `BaseClassificationDataset`, set class-level metadata.
3. Implement `is_downloaded()`, `download()`, `_iter_examples()` (yields `ClassificationExample`).
4. Decorate the class with `@register_classification`.
5. Import the new module from `src/followup_data/loaders/__init__.py`.
...
## Training a directional classifier

The `classifier` module trains a dual-encoder directional classifier that
predicts whether document A is a causal prerequisite of document B in a
tier-based dependency hierarchy. The architecture uses two untied encoders
(CLS pooling, L2 normalization), concatenates `[A; B; A-B; A*B]`, and feeds
the result through a pluggable classification head.

```bash
uv pip install '.[classification]'

# Best configuration (v6 sentence data, MLP head, fine-tuned encoders)
uv run train-classifier \
    ...
    --lr-encoder 2e-5 --lr-head 1e-3

# Linear head baseline (faster, weaker)
uv run train-classifier \
    --data-dir data/aep_causal_classification \
    --out-dir runs/classifier \
    --head-type linear --epochs 10 --batch-size 32

# Frozen encoders (linear probe — no encoder fine-tuning)
uv run train-classifier \
    --data-dir data/aep_causal_classification \
    --out-dir runs/classifier \
    --head-type mlp --freeze-encoders --epochs 5
```

The best checkpoint (`best.pt`) tracks best validation accuracy. After
training completes, the script automatically evaluates on the test split
and writes per-tier-pair accuracy breakdowns to `test_metrics.json`.

Available head types:

| Head | Architecture | Notes |
|------|-------------|-------|
| `linear` | Linear(4d, 1) | Single projection, fast baseline |
| `mlp` | LayerNorm → Linear → GELU → Dropout per layer | Configurable depth/width via `--head-hidden-dims` |

Full flag reference: `uv run train-classifier --help`.
